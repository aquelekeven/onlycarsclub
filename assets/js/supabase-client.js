(function () {
  ...
  async function upload(bucket, path, file) {
    if (!/^[a-z0-9][a-z0-9_-]*$/i.test(bucket) || !path) throw new SupabaseRequestError("Destino de upload inválido.", 400);
    const session = await getSession();
    if (!session) throw new SupabaseRequestError("Faça login para continuar.", 401);
    const response = await fetch(`${PROJECT_URL}/storage/v1/object/${encodeURIComponent(bucket)}/${path.split("/").map(encodeURIComponent).join("/")}`, {
      method:"POST",
      headers:{ apikey:PUBLISHABLE_KEY, Authorization:`Bearer ${session.access_token}`, "Content-Type":file.type || "application/octet-stream", "x-upsert":"true" },
      body:file
    });
