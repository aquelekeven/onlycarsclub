(() => {
  ...
  function renderTicketSales(query = "") {
    ...
    const kind = qs("[data-ticket-sales-kind]")?.value || "all";
    const rows = ticketSales.filter((item) => (kind === "all" || (item.ticket_kind || "expo") === kind) && (!normalized || `${item.ticket_code} ${item.driver_name} ${item.vehicle_plate} ${item.vehicle_make} ${item.vehicle_model} ${item.customer_email}`.toLowerCase().includes(normalized)));
    const count = qs("[data-ticket-sales-count]");
    if (count) count.textContent = `${rows.length} de ${ticketSales.length} ingresso(s)`;
