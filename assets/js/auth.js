(function () {
  ...
  async function loadAccount(user) {
    ...
    try {
      ...
      let originalBirthDate = profile.birth_date || "";
      ...
      const ticketKindLabel = ticket => ({expo:"Expo",carona:"Carona Radical",combo:"Expo + Carona Radical"}[ticket.ticket_kind || "expo"] || "Expo");
      const ticketCard = (ticket) => {
        ...
        const vehicleName = ticket.ticket_kind === "carona" ? "Carona Radical" : titleCase([ticket.vehicle_make, ticket.vehicle_model].filter(Boolean).join(" "));
        ...
        return `<article class="account-ticket-card ${paid ? "is-active" : ""}" data-ticket-card-id="${escapeHtml(ticket.id)}" data-ticket-code="${escapeHtml(ticket.ticket_code)}">
          <header><div><span>${escapeHtml(ticket.event_name || "Only Cars Meeting")}${ticket.is_test ? ' · <em class="qa-badge">QA</em>' : ""}</span><strong>${escapeHtml(ticket.ticket_code)}</strong><small>${escapeHtml(ticketKindLabel(ticket))} · Classificação ${escapeHtml(ticket.age_rating || "Livre")}</small></div><b data-status="${escapeHtml(ticket.ticket_status)}">${escapeHtml(expired ? "Reserva expirada" : ticketStatusLabels[ticket.ticket_status] || ticket.ticket_status)}</b></header>
          <div class="account-ticket-body"><div class="account-ticket-car"><i><svg viewBox="0 0 32 20" aria-hidden="true"><path d="M3 14.5h2.5l1.8-5.2h15.2l3.8 5.2H29v2.2h-2.2M9.2 16.7h11.9M9.5 9.3l3-4h6l4 4"/><circle cx="7.4" cy="16.1" r="2.3"/><circle cx="24.4" cy="16.1" r="2.3"/></svg></i><div><strong>${escapeHtml(vehicleName)}</strong><span>${ticket.vehicle_plate ? escapeHtml(String(ticket.vehicle_plate).toUpperCase())+" · " : ""}${escapeHtml(driverName)}</span></div></div>
          <dl><div><dt>Modalidade</dt><dd>${escapeHtml(ticketKindLabel(ticket))}</dd></div><div><dt>Valor</dt><dd>${formatMoney(ticket.total_cents)}</dd></div><div><dt>Data</dt><dd>${ticket.event_starts_at ? new Date(ticket.event_starts_at).toLocaleDateString("pt-BR") : "23/10/2026"}</dd></div><div><dt>Local</dt><dd>${escapeHtml(ticket.venue_name || "Centro de Esportes Radicais")}</dd></div></dl></div>
          <footer>${paid && ticket.qr_token ? `<section class="account-ticket-pass"><div class="account-ticket-pass-title"><div><span>Credencial digital</span><strong>${escapeHtml(ticket.event_name || "Only Cars Meeting")}</strong></div><b>${escapeHtml(ticketKindLabel(ticket))}</b></div><div class="account-ticket-pass-grid"><div><span>Ingresso</span><strong>${escapeHtml(ticket.ticket_code)}</strong></div><div><span>${ticket.ticket_kind === "carona" ? "Participante" : "Veículo"}</span><strong>${escapeHtml(ticket.ticket_kind === "carona" ? driverName : String(ticket.vehicle_plate || "").toUpperCase())}</strong></div><div><span>Data</span><strong>${ticket.event_starts_at ? new Date(ticket.event_starts_at).toLocaleDateString("pt-BR") : "23/10/2026"}</strong></div><div><span>Valor</span><strong>${formatMoney(ticket.total_cents)}</strong></div></div><div class="account-ticket-qr-wrap"><canvas data-ticket-qr="${escapeHtml(ticket.qr_token)}" aria-label="QR Code do ingresso ${escapeHtml(ticket.ticket_code)}"></canvas><p>${ticket.ticket_kind === "carona" ? "Apresente este QR Code à equipe da Carona Radical." : ticket.ticket_kind === "combo" ? "Use este QR Code na portaria e na Carona Radical." : "Apresente este QR Code na portaria."}<br>${ticket.ticket_kind !== "expo" && ticket.ticket_kind ? (ticket.carona_redeemed_at ? "Carona já utilizada." : "Carona disponível para uma utilização.") : "Não compartilhe sua credencial."}</p></div><div class="account-ticket-pass-actions"><button type="button" class="secondary" data-copy-ticket-token="${escapeHtml(ticket.qr_token)}">Copiar código</button><button type="button" class="primary" data-download-ticket="${escapeHtml(ticket.id)}">Salvar ingresso</button></div></section>${ticket.ticket_kind === "carona" ? "" : `<form class="account-ticket-photo" data-ticket-photo-form data-ticket-id="${escapeHtml(ticket.id)}"><div><strong>Foto para o post de confirmado</strong><span>Envie uma foto do carro. A equipe Only revisará antes da publicação.</span></div><label><input type="file" name="photo" accept="image/jpeg,image/png,image/webp" required><span>Escolher foto</span></label><label class="account-ticket-consent"><input type="checkbox" name="consent" required><span>Autorizo a Only Cars Club a utilizar esta foto na divulgação do evento.</span></label><button type="submit">Enviar foto</button><p data-ticket-photo-feedback></p></form>`}${helpPanel}` : `<div class="account-ticket-waiting"><strong>${expired ? "Reserva expirada" : "Estamos confirmando o pagamento"}</strong><span>${expired ? "Essa vaga já foi liberada. Faça uma nova reserva para continuar." : "Assim que o Mercado Pago aprovar, o QR Code aparecerá aqui automaticamente."}</span>${cancellable ? `<button type="button" class="account-ticket-cancel" data-cancel-ticket-order="${escapeHtml(ticket.order_id)}">Cancelar esta reserva</button>` : ""}</div>`}</footer>
        </article>`;
      };
      if (ticketsRoot) {
      ...
      const saveTicketImage = async (button) => {
        ...
        const qr = qs("[data-ticket-qr]", button.closest("[data-ticket-card-id]"));
        if (!ticket || !qr) throw new Error("Ingresso indisponível para download.");
        const output = document.createElement("canvas"); output.width=1080; output.height=1350;
        const ctx = output.getContext("2d");
        ctx.fillStyle="#f1cf19"; ctx.fillRect(0,0,1080,1350); ctx.fillStyle="#11110f"; ctx.fillRect(0,0,1080,210);
        ctx.fillStyle="#fff"; ctx.font="900 58px Arial"; ctx.fillText("ONLY CARS MEETING",72,105); ctx.fillStyle="#f1cf19"; ctx.font="700 25px Arial"; ctx.fillText(`CREDENCIAL DIGITAL • ${ticketKindLabel(ticket).toUpperCase()}`,74,155);
        ctx.fillStyle="#fff"; ctx.fillRect(48,180,984,1115); ctx.fillStyle="#777"; ctx.font="700 24px Arial";
        [["INGRESSO",92,280],[ticket.ticket_kind === "carona" ? "MODALIDADE" : "VEÍCULO",600,280],["DATA",92,420],["VALOR",600,420]].forEach(([text,x,y])=>ctx.fillText(text,x,y));
        ctx.fillStyle="#151513"; ctx.font="900 38px Arial"; ctx.fillText(ticket.ticket_code,92,330); ctx.fillText(ticket.ticket_kind === "carona" ? "CARONA" : String(ticket.vehicle_plate||"").toUpperCase(),600,330); ctx.fillText(ticket.event_starts_at?new Date(ticket.event_starts_at).toLocaleDateString("pt-BR"):"23/10/2026",92,470); ctx.fillText(formatMoney(ticket.total_cents),600,470);
        ctx.strokeStyle="#e7e5dc"; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(92,525); ctx.lineTo(988,525); ctx.stroke(); ctx.imageSmoothingEnabled=false; ctx.drawImage(qr,290,570,500,500);
        ctx.textAlign="center"; ctx.fillStyle="#151513"; ctx.font="900 28px Arial"; ctx.fillText(ticket.ticket_kind === "carona" ? "Apresente à equipe da Carona Radical" : ticket.ticket_kind === "combo" ? "QR válido para Expo e Carona Radical" : "Apresente este QR Code na portaria",540,1140); ctx.fillStyle="#777"; ctx.font="500 22px Arial"; ctx.fillText("Não compartilhe sua credencial com terceiros.",540,1180); ctx.fillStyle="#151513"; ctx.font="700 20px Arial"; ctx.fillText(`${ticket.driver_name} • ${ticket.ticket_kind === "carona" ? "Carona Radical" : ticket.vehicle_make+" "+ticket.vehicle_model}`,540,1240);
        const dataUrl = output.toDataURL("image/jpeg",.94);
        const link = document.createElement("a"); link.download=`ingresso-only-${ticket.ticket_code}.jpg`; link.href=dataUrl; link.style.display="none"; document.body.appendChild(link); link.click(); link.remove();
      };
      ticketsRoot?.addEventListener("click", async (event) => {
        ...
        if (downloadButton) {
          downloadButton.disabled=true; downloadButton.textContent="Preparando...";
          try { await saveTicketImage(downloadButton); downloadButton.textContent="Ingresso salvo"; }
          catch (error) { console.error("Falha ao salvar ingresso",error); downloadButton.textContent="Tentar novamente"; window.alert("Não foi possível salvar o ingresso neste navegador."); }
      ...
      profileForm.addEventListener("submit", async (event) => {
        ...
        const birthDateChanged = profileForm.birth_date.value !== originalBirthDate;
        if (birthDateChanged && !profileForm.birth_date_attestation.checked) return setFeedback(profileForm, "Confirme a declaração de responsabilidade para alterar a data de nascimento.");
        setSubmitting(profileForm, true, "Salvando...");
        setFeedback(profileForm, "");
        try {
          if (birthDateChanged) await client.rest("rpc/customer_update_birth_date", { method:"POST", body:{ p_birth_date:profileForm.birth_date.value, p_attested:true } });
          ...
          originalBirthDate = profileForm.birth_date.value;
          profileForm.birth_date_attestation.checked = false;
