...
Deno.serve(async(req)=>{
 ...
 for(const row of rows||[]){
  ...
  if(row.template==="ticket_purchase"){
   ...
   const cards=ticketItems.map((ticket:any,index:number)=>`<div style="margin:${index?"14px":"0"} 0 0;padding:20px;background:#fff;border-radius:16px"><b>${esc(ticket.ticket_code)}</b><p>${esc(ticket.vehicle_plate)}${ticket.vehicle_make?` · ${esc(ticket.vehicle_make)} ${esc(ticket.vehicle_model)}`:""}</p><img src="cid:ticket-qr-${index+1}" width="250" style="display:block;margin:20px auto"><p style="text-align:center;font-size:12px">QR Code individual do veículo ${index+1}.</p></div>`).join("");
