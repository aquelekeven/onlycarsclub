...
Deno.serve(async(req)=>{
 ...
 for(const row of rows||[]){
  ...
  else if(row.template==="ticket_photo_reminder") html=shell("Quer aparecer no post de confirmados?",`${p.driver_name}, seu ingresso está garantido — agora falta mostrar o carro para a comunidade.`,`<div style="padding:20px;background:#fff;border-radius:16px"><b>${esc(p.ticket_code)}</b><p>${esc(p.vehicle_plate)} · ${esc(p.vehicle_make)} ${esc(p.vehicle_model)}</p><p>Envie uma foto do carro para a equipe avaliar e usar no post de confirmados do evento.</p></div><p><a href="${SITE}/minha-conta.html" style="display:inline-block;background:#efd311;color:#111;padding:14px 22px;border-radius:12px;text-decoration:none;font-weight:bold">Enviar foto do carro</a></p><p style="font-size:12px;color:#777">Se você já enviou a foto recentemente, pode ignorar esta mensagem.</p>`,EVENT_IMAGE,true);
