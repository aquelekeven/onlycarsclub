...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { error:ticketError } = await admin.from("tickets").insert({ order_id:order.id, event_id:event.id, owner_user_id:user.id, qr_token:token, qr_token_hash:hash, status:"reserved", driver_name:driverName, driver_tax_id:cpf, driver_phone:phone, vehicle_plate:plate, vehicle_make:make, vehicle_model:model, instagram_handle:clean(body.instagram_handle,40)||null });
